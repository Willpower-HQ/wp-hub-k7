(async () => {
  ...
  const setDone = (eid, tid, val) => { const s = st(eid); s.done = s.done || {}; if (val) s.done[tid] = 1; else delete s.done[tid]; save(eid, s); rebuild(); };
  ...
  const cbx = t => '<span class="cbx" data-eid="' + E(t.eid) + '" data-tid="' + E(t.id) + '">&#10003;</span>';
  ...
  function wire(root) {
    root.querySelectorAll('.cbx[data-tid]').forEach(b => b.onclick = () => setDone(b.dataset.eid, b.dataset.tid, !st(b.dataset.eid).done || !st(b.dataset.eid).done[b.dataset.tid]));
  ...
  function renderDay(ds) {
    ...
    const taskRows = tasks.map(t => '<div class="dayrow' + (t.done ? ' done' : '') + '">' + cbx(t) + '<span class="ti">' + E(t.title) + '<div class="ev">' + E((eventById[t.eid] || {}).name || '') + '</div></span>' + asgSel(t) + '</div>').join('');
  ...
  function renderBoard() {
    ...
    const rows = list.map(t => {
      ...
      return head + '<div class="task' + (t.done ? ' done' : '') + '">' + cbx(t)
        + '<span class="ti">' + E(t.title) + '</span>' + asgSel(t)
    ...
    document.getElementById('tasks').innerHTML =
      '<div class="taskhead"><h2>Checklist</h2><span class="taskprog"><select id="boardEv" style="margin-right:10px">' + evOpts + '</select><b>' + doneN + '</b> of ' + list.length + ' done</span></div>'
  ...
  document.getElementById('banners').innerHTML = '<div class="banner">Tap any day to see exactly what is due and add tasks. Yellow blocks are Willpower events; dots are to-dos.</div>';
